...
class ball_spawn(Node):
    def __init__(self):
        ...
        # share_dir = get_package_share_directory('rbcon_sim')

        # Paths to configuration files
        # self.file_name = PathJoinSubstitution([share_dir, "meshes", "basketball.dae"])
    ...
    def tf_lookup_timer_callback(self):
        try:
            tf=self.tf_buffer.lookup_transform( 'robot1_base_link',
                                'base_footprint',
                                rclpy.time.Time())
    ...
    def spawn_ball(self,file_name,model,pose_x,pose_y,pose_z,orie_x=0,orie_y=0,orie_z=0,orie_w=0):
        ...
        try:
            ret = os.system(f'ros2 run ros_gz_sim create --file {file_name} -x {-pose_x} -y {-pose_y} -z {pose_z} -name {model}')
        except:
